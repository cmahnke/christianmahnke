---
date: 2026-04-06T13:30:44+01:00
title: 'Etching by Max Liebermann (1847-1935)'
tags:
  - Art
  - Etching
preview: img/print.jpg
outputs:
  - bibtex
  - html
  - linkedart
linkedart:
  artist:
    wikidata: https://www.wikidata.org/wiki/Q158062
  owner:
    wikidata: https://www.wikidata.org/wiki/Q137668638
wikidata:
  - https://www.wikidata.org/wiki/Q158062
  - https://www.wikidata.org/wiki/Q84957520
---

I have bought an etching by [Max Liebermann](https://en.wikipedia.org/wiki/Max_Liebermann) (1847–1935) ([at artnet](https://www.artnet.com/artists/max-liebermann/))...
<!--more-->

{{< gallery >}}
[
  {"src": "img/print.jpg", "alt": Print"}
]
{{</ gallery >}}

The etching is entitled "Am Strande" and depicts a beach scene in [Noordwijk](https://en.wikipedia.org/wiki/Noordwijk). Liebermann often spent the summer months there. The print is a drypoint etching. It appeared as sheet 4 in the 1915 portfolio ‘Freunde Graphischer Kunst’.
It appears to be neither signed nor numbered. Various versions can be found online, including signed and unsigned copies, as well as those that are heavily faded. It is therefore impossible to determine whether this is an original without removing it from its frame.

The [Lindenau Museum in Altenburg](https://guide.lindenau-museum.de/menschenleben-audio/max-liebermann) also dedicates an entry to the print in its audio guide.

Max Liebermann was a major pioneer of German modernism and, as the long-standing president of the [Berliner Secession](https://en.wikipedia.org/wiki/Berlin_Secession), shaped the transition from realism to [Impressionism](https://en.wikipedia.org/wiki/Impressionism). As a Jewish artist, he was banned from exhibiting by the Nazis and forced into social isolation, which ultimately compelled him to resign himself to withdrawing from the Academy of Arts, which he had helped to found.

Von ihm stammt der Satz: "Ick kann jar nich soville fressen, wie ick kotzen möchte." (I can't eat as much as I'd like to throw up)
